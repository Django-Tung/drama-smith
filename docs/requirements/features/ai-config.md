...
每个用户**自带模型凭证(BYOK)**,不依赖平台统一密钥。**首次登录强制配置文本模型**(图片/视频可选):未配置图片/视频时,相应的分镜视觉素材/图(FR-A7)与逐镜视频生成(FR-A8)不可用,但**不阻塞进入主功能**。文本拆解始终可用;三类模型的实际调用见 [`analysis.md`](./analysis.md)。
...
| 编号 | 需求 | 说明 |
|------|------|------|
| FR-C1 | 首次登录强制配置文本 | 新用户首次登录**必须配置文本模型**方可进入主功能;图片/视频可选,未配置则禁用 FR-A7/A8(详见 §2.2、[`analysis.md`](./analysis.md)) |
...
1. **首次配置严格度** → **文本必配,图片/视频可选**:未配置则禁用 FR-A7/A8,不阻塞进入主功能(已写入 FR-C1、§1、§4)。
