...
Defines the user-authentication capability for Drama Smith: username/password registration, login, logout, and token refresh. Passwords are stored as argon2id hashes; authentication issues short-lived JWT (HS256) access tokens plus server-revocable opaque refresh tokens, enforces a per-account brute-force lockout, and isolates every user-owned record by `user_id` (multi-tenant).
